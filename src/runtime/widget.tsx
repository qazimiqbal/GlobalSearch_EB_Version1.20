...
> {
  ...
  scaleThreshold = 5000;
  onActiveViewChange = (jimuMapView: JimuMapView, isActive: boolean, active: boolean) => {    
    ...
    console.log(active ? "Widget is Active" : "Widget is Inactive");

    //this.setState({ isActive });
    //console.log(isActive ? "Widget is Active" : "Widget is Inactive");
  ...
  getdataFromMapService = async (addressInput: string) => {
    ...
    try {
      ...
      const params = {
        // Search only records that START with the input text (prefix match)
        where: `Name LIKE '${addressInput}%' OR ${displayField} LIKE '${addressInput}%'`,
        outFields: '*',
        returnGeometry: true,
        f: 'json'
      };
